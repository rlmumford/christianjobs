...
namespace Drupal\contacts_jobs_dashboard\Access;
...
class RoleAccessCheck implements AccessInterface {
   ...
   *   The access result.
   */
  public function access(Route $route, AccountInterface $account, UserInterface $user = NULL) {
    if (empty($user)) {
    ...
    $rid_string = $route->getRequirement('_dashboard_role');

    $explode_and = array_filter(array_map('trim', explode(',', $rid_string)));
    if (count($explode_and) > 1) {
      $diff = array_diff($explode_and, $user->getRoles());
      if (empty($diff)) {
      ...
      $intersection = array_intersect($explode_or, $user->getRoles());
